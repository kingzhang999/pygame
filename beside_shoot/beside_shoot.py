...
class Beside_Shoot:
	...
	def update_bullet(self):
		...
		#删除消失的子弹
		if self.yuanlai:
			self.sprites = self.bullets_common
		elif self.xinzidan:
			self.sprites = self.bullets_armour
		elif self.gaobao:
			self.sprites = self.bullets_explosive
		for bullete in self.sprites.copy():#遍历编组的副本
			if bullete.rect.x >= self.screen_rect.right:
				self.sprites.remove(bullete)
	...
	def check_bullet_alien_collisions(self):
		...
		if self.yuanlai:
			...
			self.sprites = self.bullets_explosive
		collision = pygame.sprite.groupcollide(self.sprites,self.aliens,
	...
	def update_screen(self):
		...
		if self.yuanlai:
			...
			self.sprites = self.bullets_explosive
		for bullet in self.sprites.sprites():#方法sprites()返回一个列表,包含编组内所有元素
